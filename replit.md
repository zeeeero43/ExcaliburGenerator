...
- **Core Features**:
    ...
    - **Security**: Minimal security approach focusing on essential protection only. MIME and magic number validation for uploads, input sanitization, security headers (Helmet), Zod schema validation, and simple region blocking (China & Singapore only). **MAJOR SIMPLIFICATION (Aug 2025)**: Bot protection drastically reduced to only country-blocking. All rate limiting, user-agent detection, violation tracking, and complex bot detection removed. Only China (CN) and Singapore (SG) are blocked - all other countries have unrestricted access. Admin login rate limiting set to 50 attempts per 15 minutes.
