export const translations = {
  es: {
    // Navigation
    home: "Inicio",
    products: "Productos",
    about: "Nosotros",
    contact: "Contacto",
    searchProducts: "Buscar productos...",
    
    // Hero Section
    heroTitle1: "ENERGÍA SOLAR PARA CUBA",
    heroSubtitle1: "Componentes solares y generadores de alta calidad al mejor precio",
    heroTitle2: "GENERADORES EXCALIBUR",
    heroSubtitle2: "Generadores diesel y gasolina silenciosos de 2-20KVA",
    heroTitle3: "SOLUCIONES COMPLETAS",
    heroSubtitle3: "Sistemas solares personalizados según sus necesidades",
    viewProducts: "Ver Productos",
    inquireNow: "Consultar Ahora",
    customizeSystem: "Personalizar Sistema",
    
    // Features
    bestQuality: "Mejor Calidad al Mejor Precio",
    bestQualityDesc: "Productos certificados internacionalmente al mejor precio",
    fastDelivery: "Rápida Disponibilidad",
    fastDeliveryDesc: "Schnelle Verfügbarkeit bei Selbstabholung - Habana del Este",
    technicalSupport: "Soporte Técnico",
    technicalSupportDesc: "Asesoría especializada 24/7",
    
    // Products
    ourProducts: "NUESTROS PRODUCTOS",
    productsSubtitle: "Gran oferta de componentes solares certificados, sistemas solares completos, generadores, materiales de construcción y otros materiales y mucho más.",
    selectCategory: "Selecciona una categoría",
    selectSubcategory: "Selecciona una subcategoría",
    backToCategories: "Volver a Categorías",
    completeSolarSystems: "Sistemas Solares Completos",
    completeSolarSystemsDesc: "Sistemas preconfigurados de 1KW a 20KW con inversor híbrido, paneles monocristalinos y opciones con/sin batería. Ideales para hogares y negocios en Cuba con garantía internacional.",
    solarPanels: "Paneles Solares",
    solarPanelsDesc: "Paneles fotovoltaicos monocristalinos y policristalinos de 300W a 550W con certificación IEC y garantía. Máxima eficiencia para el clima tropical cubano.",
    inverters: "Inversores",
    invertersDesc: "Inversores híbridos y de red de 1KW a 20KW con MPPT integrado, compatible con baterías de litio. Tecnología alemana con protecciones avanzadas.",
    batteries: "Baterías",
    batteriesDesc: "Baterías de litio LiFePO4 y AGM de ciclo profundo desde 100Ah hasta 1000Ah. Diseñadas para sistemas solares con más de 6000 ciclos de vida útil.",
    generators: "Generadores",
    generatorsDesc: "Generadores diesel y gasolina silenciosos Excalibur de 2KVA a 20KVA con arranque eléctrico, bajo consumo y nivel de ruido inferior a 60dB.",
    constructionMachines: "Materiales de construcción y otros materiales",
    constructionMachinesDesc: "Ofrecemos materiales de construcción y otros materiales para todos los proyectos en Cuba. Tecnología de alta calidad para aplicaciones profesionales.",
    accessories: "Accesorios",
    accessoriesDesc: "Kit completo de instalación: estructuras de aluminio, cables solares, fusibles, conectores MC4, sistemas de puesta a tierra y herramientas especializadas.",
    viewSystems: "Ver Sistemas →",
    viewPanels: "Ver Paneles →",
    viewInverters: "Ver Inversores →",
    viewBatteries: "Ver Baterías →",
    viewGenerators: "Ver Generadores →",
    viewAccessories: "Ver Accesorios →",
    
    // About - Neue spanische Übersetzung
    aboutTitle: "ACERCA DE NOSOTROS - EXCALIBUR POWER CUBA",
    aboutText1: "Somos una MIPYME de Matanzas y representamos al Grupo Excalibur Power en Cuba en colaboración con nuestro patrocinador y proveedor alemán. Excalibur pertenece a los fabricantes líderes mundiales de generadores, energía solar y muchos otros componentes.",
    aboutText2: "Garantizamos la mejor calidad al mejor precio gracias a nuestro socio Joint Venture, patrocinador y proveedor alemán con 35 años de experiencia en el mercado. Nuestro almacén se encuentra en La Habana del Este y está ubicado favorablemente para el tráfico, para abastecer a toda Cuba con entregas rápidas y un gran stock de almacén. Nuestro proveedor trabaja directamente con las fábricas para garantizar los mejores precios y la más alta calidad.",
    yearsExperience: "Años de experiencia",
    solarSystems: "Sistemas solares",
    ourWarehouse: "Nuestro Almacén en Cuba",
    warehouseDesc: "Nuestro almacén se encuentra en La Habana del Este y está ubicado favorablemente para el tráfico, y está ocupado 24/7, para ser fácilmente accesible para clientes de toda Cuba. Disponemos de un abundante stock de almacén. Nuestro proveedor trabaja directamente con los fabricantes de todo el mundo para garantizar siempre los mejores precios y la más alta calidad. Nuestros productos tienen una garantía de mejor precio y el stock es enorme, ya que nuestro proveedor compra todo en grandes cantidades y envía permanentemente contenedores completos desde Europa y Asia a Cuba.",
    
    // Customer Satisfaction - Nueva Sección
    customerSatisfactionTitle: "Satisfacción del Cliente",
    customerSatisfactionDesc: "La satisfacción del cliente es muy importante para nosotros e incluye no solo la mejor calidad al mejor precio, sino también un soporte técnico 24/7 que es único en Cuba. En +49 160 323 9439 recibe una respuesta competente a todas sus preguntas.",
    jointVenturePartnerTitle: "Nuestro Socio Joint Venture y Proveedor",
    jointVenturePartnerSubtitle: "AFDL IMPORT & EXPORT GMBH & CO.KG",
    jointVenturePartnerDesc: "Con 35 años de experiencia en los mercados internacionales y sede en Alemania, nuestro socio Joint Venture permite la importación de grandes cantidades de productos a precios favorables, para que podamos transferir estos productos a precios favorables a nuestros clientes cubanos. Hemos aprendido en Alemania cómo funciona la economía de mercado.",
    
    // Additional About translations
    ourHistory: "Nuestra Historia",
    satisfiedClients: "Clientes satisfechos",
    technicalSupportStats: "Soporte técnico",
    guaranteedQuality: "Mejor calidad al mejor precio",
    guaranteedQualityDesc: "Productos certificados internacionalmente con garantías extendidas",
    expertTeam: "Equipo Experto",
    expertTeamDesc: "Técnicos especializados con más de 10 años de experiencia",
    bestPrice: "Mejor Precio",
    bestPriceDesc: "Compras directas de fábrica para ofrecer los mejores precios",
    internationalReach: "Alcance Internacional",
    internationalReachDesc: "Respaldados por AFDL Import & Export de Alemania",
    whyChooseUs: "¿Por qué elegir Excalibur Cuba?",
    strategicPartner: "Nuestro Socio Estratégico",
    partnershipDesc: "Con 35 años de experiencia en los mercados internacionales y sede en Alemania, nuestro socio Joint Venture permite la importación de grandes cantidades de productos a precios favorables, para que podamos transferir estos productos a precios favorables a nuestros clientes cubanos. Hemos aprendido en Alemania cómo funciona la economía de mercado.",
    customerSatisfaction: "Satisfacción del Cliente",
    countriesServed: "Países servidos",
    containersSent: "Contenedores enviados",
    warehouseLocation: "Nuestro almacén en Havanna del Este está estratégicamente ubicado para servir a toda Cuba con entregas rápidas y stock abundante. Trabajamos directamente con las fábricas para garantizar los mejores precios y la más alta calidad.",
    heroSubtitle: "Somos una MIPYME de Matanzas y representamos al Grupo Excalibur Power en Cuba en colaboración con nuestro patrocinador y proveedor alemán.",
    
    // Contact
    contactUs: "CONTÁCTANOS",
    contactSubtitle: "Configuramos el sistema solar perfecto para sus necesidades",
    contactInfo: "Información de Contacto",
    technicalAdvisory: "Asesoría Técnica & Ventas",
    technicalAdvisoryPhone: "+49 160 323 9439",
    salesPhone: "+53 5878 1416",
    deliveryPhone: "+53 54 73 14 90",
    whatsappAvailable: "WhatsApp disponible",
    supportSales: "Soporte+Ventas",
    administrationSales: "Administración + Ventas",
    deliveryPickup: "Entrega / Recogida",
    
    // Cart System
    cart: "Carrito",
    addToCart: "Agregar al Carrito",
    viewCart: "Ver Carrito",
    cartEmpty: "Tu carrito está vacío",
    cartEmptyDesc: "Agrega productos a tu carrito para continuar",
    continueWhatsApp: "Continuar con WhatsApp",
    removeFromCart: "Eliminar",
    clearCart: "Limpiar todo",
    quantity: "Cantidad",
    total: "Total",
    subtotal: "Subtotal",
    cartItems: "artículos",
    myCart: "Mi Carrito",
    sendViaWhatsApp: "Enviar por WhatsApp",
    
    // Mobile Image Gallery
    showMobileImages: "Ver todas las imágenes",
    hideMobileImages: "Ocultar imágenes",
    whatsAppMessage: "Hola, me interesan estos productos:",
    administration: "Administración",
    warehouseDelivery: "Recogida / Almacén",
    salesLabel: "Ventas",
    emailAddresses: "Correos Electrónicos",
    requestQuote: "Solicitar Cotización",
    name: "Nombre",
    phone: "WhatsApp Número",
    email: "Email",
    systemType: "Tipo de Sistema",
    select: "Seleccionar...",
    completeSolarSystem: "Sistema Solar Completo",
    generatorOnly: "Solo Generador",
    individualComponents: "Componentes Individuales",
    message: "Mensaje",
    messagePlaceholder: "Describa sus necesidades energéticas...",
    sendRequest: "Enviar Solicitud",

    // Product Detail Page
    currentLanguage: "es",
    productNotFound: "Producto no encontrado",
    backToProducts: "Volver a productos",
    description: "Descripción",
    specifications: "Especificaciones",
    inStock: "En Stock",
    limitedStock: "Stock Limitado",
    outOfStock: "Agotado",
    interestedInProduct: "¿Interesado en este producto?",
    contactWhatsApp: "Contactar por WhatsApp",
    contactEmail: "Enviar Email",
    contactPhone: "Llamar",
    qualityGuarantee: "Garantía de Calidad",
    expertSupport: "Soporte Experto",
    germanQuality: "Calidad alemana certificada",
    monthlyContainerImports: "Importaciones mensuales de contenedores",
    bestPrices: "Los mejores precios del mercado",
    
    // SEO Content Section
    solarEnergyTitle: "Solarenergie in Kuba angesichts der langanhaltenden Probleme in der Energieversorgung",
    solarEnergyDescription: "Ein Muss für alle Kubaner - Cuba se encuentra en una posición geográfica privilegiada para el aprovechamiento de la energía solar, con más de 330 días de sol al año y una irradiación solar promedio de 5.5 kWh/m²/día.",
    solarAdvantagesTitle: "Vorteile einer Solaranlage",
    solarAdvantageEnergyIndependence: "Energieunabhängigkeit für Geschäft und zu Hause",
    solarAdvantageReliableTech: "Zuverlässige, neueste Technologie mit Garantie",
    solarAdvantageLowMaintenance: "Geringer Wartungsaufwand",
    solarAdvantageLongLifetime: "Lange Lebensdauer",
    solarAdvantageEnvironment: "Ein Beitrag zum Schutz der Umwelt",
    solarAdvantage1: "Conviértase en su propio productor de electricidad",
    solarAdvantage2: "Independícese de la red eléctrica pública y sus problemas asociados",
    solarAdvantage3: "Para su negocio y su hogar",
    solarAdvantage4: "Energía gratuita del sol",
    solarAdvantage5: "Contribución al cuidado del medio ambiente",
    ourSystemsTitle: "Nuestros Sistemas Solares",
    systemFeature1: "Sistemas completos de 3KW a 30KW",
    systemFeature2: "Módulos solares certificados de alta eficiencia para montaje en y sobre edificios",
    systemFeature3: "Inversores híbridos con respaldo de batería",
    systemFeature4: "Baterías LiFePO4 de larga duración",
    systemFeature5: "Instalación y soporte técnico especializado",
    
    // Additional SEO Content
    generatorsTitle: "Generadores de Respaldo: Seguridad Energética Garantizada",
    generatorsDescription: "Complementamos nuestros sistemas solares con generadores silenciosos de última generación, proporcionando una solución energética integral que garantiza suministro eléctrico continuo. Nuestros generadores diesel y gasolina ofrecen desde 2KVA hasta 20KVA, ideales para hogares, pequeños negocios y aplicaciones industriales.",
    whyChooseTitle: "¿Por qué elegir Excalibur Power Cuba?",
    whyChooseDescription: "Somos una empresa joven en Cuba. Con el apoyo de nuestra empresa asociada alemana AFDL IMPORT & EXPORT, con 35 años de experiencia profesional internacional, podemos ofrecer la mejor calidad al mejor precio. Amamos nuestro país Cuba y nuestro compromiso es iluminar el futuro de Cuba con tecnología de vanguardia y servicio profesional.",
    ourSolarOffers: "Unsere Solar-Angebote",
    ourSolarOffersDesc: "(Vertrieb) Solar Komponenten & komplette Systeme, Großangebot hocheffizienter monokristalliner Solar Paneele, Hybride Wechselrichter, Lithium Batterien, All in One Geräte + sämtliches Zubehör",
    backupGenerators: "Backup-Generatoren",
    backupGeneratorsDesc: "Von 2,0 KW – 20KW",
    monthlyImportsTitle: "Importaciones Permanentes",
    monthlyImportsDescription: "No escatimamos costos ni esfuerzos, a través de investigación permanente e importaciones de tecnologías innovadoras de todo el mundo, para contribuir a tener disponibles los componentes que nuestros clientes y futuros clientes van a demandar.",
    contactUsHeader: "Kontaktieren Sie uns",
    contactUsDesc: "Gern stellen wir Ihnen die gewünschten Komponenten inkl. einem attraktiven Preisangebot zusammen",
    illuminateCuba: "Wir erhellen Cuba besser",
    illuminateCubaDesc: "Für eine hellere Zukunft in Cuba",
    languagesSpoken: "Sprachen",
    languagesSpokenDesc: "Wir sprechen: Spanisch, Englisch, Deutsch",
    
    // Footer
    quickLinks: "Enlaces Rápidos",
    allRightsReserved: "Todos los derechos reservados.",
    companyDescription: "Importador profesional de componentes solares y generadores en Cuba. Representantes oficiales de Excalibur Power Group.",
    
    // WhatsApp
    needHelp: "¿Necesita ayuda?",
    whatsapp: "WhatsApp",
    
    // Form Messages
    formSubmitted: "Formulario enviado. Nos pondremos en contacto pronto.",
    formError: "Error al enviar formulario. Intente nuevamente.",
    
    // Products Page
    allCategories: "Todas las categorías",
    noProductsFound: "No se encontraron productos",
    noProductsMessage: "No hay productos disponibles en esta categoría.",
    viewDetails: "Ver Detalles",
    subcategories: "Subcategorías",
    subcategory: "Subcategoría", 
    category: "Categoría",
    product: "Producto",
    moreProducts: "Más Productos",
    noContentFound: "No se encontraron contenidos",
    noCategoryContent: "Esta categoría no contiene actualmente subcategorías o productos.",
    productsMetaDesc: "Productos Excalibur Cuba: Sistemas solares completos, generadores, baterías litio, inversores, cables MC4, excavadoras, compresores, filtros agua, hornos pizza en Matanzas.",
    productsMetaKeywords: "productos Excalibur Cuba, sistemas solares Cuba, generadores Cuba, baterías litio Cuba, inversores Cuba, cables MC4 Cuba, excavadoras Cuba, compresores Cuba, Matanzas",
    featured: "Destacado",
    in_stock: "En stock",
    limited: "Stock limitado",
    out_of_stock: "En tránsito",
    onTheWay: "En camino",
    seaTransport: "En el mar",
    priceOnRequest: "Precio bajo consulta",
    onSale: "En oferta",
    wattPeak: "Vatio Pico",
    needCustomSolution: "¿Necesitas una solución personalizada?",
    customSolutionDescription: "Nuestros expertos pueden diseñar un sistema solar específico para tus necesidades energéticas.",
    
    // Offline and Cookie Banner
    offline: {
      message: 'Sin conexión a internet'
    },
    cookieBanner: {
      title: 'Uso de Cookies',
      description: 'Utilizamos cookies para mejorar su experiencia en nuestro sitio web.',
      acceptAll: 'Aceptar todas',
      rejectAll: 'Rechazar todas',
      settings: 'Configuración',
      settingsTitle: 'Preferencias de Cookies',
      necessary: 'Cookies necesarias',
      necessaryDesc: 'Estas cookies son esenciales para el funcionamiento del sitio web.',
      analytics: 'Cookies de análisis',
      analyticsDesc: 'Nos ayudan a entender cómo utilizas nuestro sitio web.',
      marketing: 'Cookies de marketing',
      marketingDesc: 'Se utilizan para mostrar anuncios relevantes.',
      acceptSelected: 'Aceptar seleccionadas',
      required: 'Requeridas'
    },
    
    // Product Detail Extensions
    productFeatures: "Características del Producto",
    qualityAssurance: "Garantía de Calidad",
    qualityAssuranceDesc: "Productos certificados internacionalmente con garantías extendidas",
    highPerformance: "Alto Rendimiento",
    highPerformanceDesc: "Tecnología de vanguardia para máxima eficiencia energética",
    warrantyDesc: "Garantías internacionales extendidas",
    installationSetup: "Instalación y Configuración",
    professionalInstallation: "Instalación Profesional",
    professionalInstallationDesc: "Nuestro equipo especializado se encarga de la instalación completa de tu sistema solar.",
    installationStep1: "Evaluación técnica del sitio y dimensionamiento del sistema",
    installationStep2: "Instalación profesional de paneles, inversor y baterías",
    installationStep3: "Configuración y puesta en marcha del sistema completo",
    installationStep4: "Capacitación al usuario y entrega de documentación",
    technicalSupportExtended: "Soporte técnico especializado disponible las 24 horas del día.",
    supportChannels: "Canales de Soporte",
    technicalDetails: "Detalles Técnicos",
    certifications: "Certificaciones",
    certified: "Certificado",
    ceCompliance: "Conformidad CE",
    iecStandard: "Estándar IEC 61215",
    rohs: "Conformidad RoHS",
    iso9001: "ISO 9001:2015",
    warranty: "Garantía",
    manufacturerWarranty: "Garantía del Fabricante",
    manufacturerWarrantyDesc: "Garantía extendida de producto contra defectos de fabricación",
    performanceWarranty: "Garantía de Rendimiento",
    performanceWarrantyDesc: "Garantía de potencia mínima del 80%",
    supportWarranty: "Garantía de Soporte",
    supportWarrantyDesc: "Soporte técnico gratuito durante toda la vida útil del producto",
    aboutExcaliburCuba: "Sobre Excalibur Cuba",
    aboutExcaliburDesc: "Representantes oficiales de Excalibur Power Group en Cuba, ofreciendo soluciones energéticas de alta calidad para iluminar el futuro de Cuba.",
    
    // Missing keys for Home page  
    ourServices: "Nuestros Servicios",
    contactDesc: "Con gusto le armamos el sistema solar que se adapte a sus necesidades",
    international: "Internacional",
    warehouse: "Recogida / Almacén",
    havanaLocation: "Habana del Este",
    emailLabel: "Contacto por Email:",
    
    // Home page boxes
    experience: "35 años de experiencia",
    experienceDesc: "Nuestro proveedor alemán AFDL cuenta con 35 años de experiencia internacional",
    directImport: "Importaciones directas de todo el mundo a Cuba",
    directImportDesc: "Entregas permanentes de componentes solares y generadores de alta calidad desde Alemania y centros de fabricación mundiales",
    homeLocation: "Ubicación central estratégica",
    homeLocationDesc: "Almacén en Habana del Este para disponibilidad rápida con recogida",
    
    // Services
    components: "Componentes Solares Individuales",
    solarSystemsDesc: "Sistemas solares completos de 3 a 30KW con componentes de alta calidad al mejor precio",
    componentsDesc: "Con nosotros obtiene todo para armar cualquier sistema solar según necesidades y deseos, configurado individualmente",
    constructionMaterials: "Materiales de construcción y otros materiales",
    constructionMaterialsDesc: "Materiales de construcción de alta calidad para sus proyectos",
    ourServicesForYou: "Unsere Leistungen für Sie",
    ourServicesForYouDesc: "Wir bieten: Solar-Komponenten, komplette Solaranlagen, Generatoren und mehr",
    generatorsVariety: "Generatoren: Wir bieten Benzin & Diesel Generatoren in vielen verschiedenen Varianten 2-20KVA",
    buildingMaterials: "Bau-Materialien: Carbon Glasfaser Bewehrung, Marmor Balustraden, Schiebetore & vieles mehr",
    
    // Product page translations
    productCount: "productos",
    inThisCategory: "en esta categoría",
    
    // Contact page
    contactLocation: "Ubicación",
    contactLocationTitle: "Nuestra Ubicación",
    contactLocationDesc: "Almacén principal en Havanna del Este con fácil acceso desde toda Cuba",
    
    // About
    slogan: "Para un futuro más brillante en Cuba",
    
    // Admin Dashboard Top-Level Keys
    dashboard: "Admin Excalibur Cuba",
    welcome: "Bienvenido",
    analytics: "Análisis",
    websiteImages: "Imágenes del Sitio",
    imageManager: "Gestión de Imágenes",
    viewWebsite: "Ver Sitio Web",
    categories: "Categorías",
    activeCategories: "Categorías Activas",
    activeProducts: "Productos Activos",
    inquiries: "Consultas",
    newInquiries: "Consultas Nuevas",
    settings: "Configuración",
    systemSettings: "Configuración del Sistema",
    recentProducts: "Productos Recientes",
    recentCategories: "Categorías Recientes",
    recentInquiries: "Consultas Recientes",
    addProduct: "Agregar Producto",
    addCategory: "Agregar Categoría",
    edit: "Editar",
    delete: "Eliminar",
    view: "Ver",
    nameField: "Nombre",
    statusField: "Estado",
    emailField: "Correo",
    messageField: "Mensaje",
    priceField: "Precio",
    categoryField: "Categoría",
    active: "Activo",
    inactive: "Inactivo",
    new: "Nuevo",
    replied: "Respondido",
    online: "En Línea",
    systemRunning: "Sistema funcionando",
    manageProducts: "Gestionar Productos",
    manageCategories: "Gestionar Categorías",
    customerInquiries: "Consultas de Clientes",
    productManagement: "Gestión de Productos",
    productManagementDesc: "Gestione sus productos de forma fácil y rápida",
    categoryManagement: "Gestión de Categorías",
    categoryManagementDesc: "Organice sus categorías de productos",
    inquiryManagement: "Gestión de Consultas",
    inquiryManagementDesc: "Procese consultas de clientes",
    noProductsAvailable: "No hay productos disponibles",
    noCategoriesAvailable: "No hay categorías disponibles",
    noInquiriesAvailable: "No hay consultas disponibles",
    image: "Imagen",
    stockStatus: "Estado de Stock",
    availability: "Disponibilidad",
    createdAt: "Creado el",
    actions: "Acciones",
    
    // Admin Backend
    admin: {
      // Login
      loginTitle: "Iniciar Sesión",
      loginSubtitle: "Acceda al panel de administración",
      username: "Usuario",
      password: "Contraseña",
      loginButton: "Iniciar Sesión",
      loginError: "Usuario o contraseña incorrectos",
      
      // Dashboard
      dashboard: "Panel de Control",
      overview: "Resumen",
      totalProducts: "Productos Totales",
      totalCategories: "Categorías Totales", 
      pendingInquiries: "Consultas Pendientes",
      activeProducts: "Productos Activos",
      recentProducts: "Productos Recientes",
      recentCategories: "Categorías Recientes",
      recentInquiries: "Consultas Recientes",
      viewAll: "Ver Todos",
      
      // Products
      products: "Productos",
      productManagement: "Gestión de Productos",
      currentProducts: "Productos Actuales",
      newProduct: "Nuevo Producto",
      addProduct: "Agregar Producto",
      editProduct: "Editar Producto",
      productName: "Nombre del Producto",
      productDescription: "Descripción",
      productSpecifications: "Especificaciones",
      productPrice: "Precio",
      productStock: "Disponibilidad",
      productImages: "Imágenes",
      productCategory: "Categoría",
      productSubcategory: "Subcategoría",
      selectCategory: "Seleccionar categoría",
      selectSubcategory: "Seleccionar subcategoría",
      inStock: "En Stock",
      limitedStock: "Stock Limitado",
      outOfStock: "Agotado",
      saveProduct: "Guardar Producto",
      
      // Categories
      categories: "Categorías",
      addCategory: "Agregar Categoría",
      editCategory: "Editar Categoría",
      categoryName: "Nombre de Categoría",
      categoryDescription: "Descripción",
      categoryImage: "Imagen",
      saveCategory: "Guardar Categoría",
      
      // Subcategories  
      subcategories: "Subcategorías",
      addSubcategory: "Agregar Subcategoría",
      editSubcategory: "Editar Subcategoría",
      subcategoryName: "Nombre de Subcategoría",
      parentCategory: "Categoría Principal",
      saveSubcategory: "Guardar Subcategoría",
      
      // Inquiries
      inquiries: "Consultas",
      inquiryName: "Nombre",
      inquiryEmail: "Email",
      inquiryPhone: "Teléfono",
      inquirySubject: "Asunto",
      inquiryMessage: "Mensaje",
      inquiryStatus: "Estado",
      inquiryDate: "Fecha",
      statusNew: "Nueva",
      statusInProgress: "En Progreso",
      statusCompleted: "Completada",
      
      // Images
      images: "Imágenes",
      uploadImages: "Subir Imágenes",
      selectImage: "Seleccionar Imagen",
      imageGallery: "Galería de Imágenes",
      
      // Analytics
      analytics: "Analíticas",
      totalViews: "Vistas Totales",
      uniqueVisitors: "Visitantes Únicos",
      topPages: "Páginas Principales",
      topCountries: "Países Principales",
      
      // Common
      actions: "Acciones",
      edit: "Editar",
      delete: "Eliminar",
      cancel: "Cancelar",
      save: "Guardar",
      back: "Volver",
      backToDashboard: "Volver al Panel",
      search: "Buscar",
      filter: "Filtrar",
      all: "Todos",
      yes: "Sí",
      no: "No",
      loading: "Cargando...",
      success: "Éxito",
      error: "Error",
      confirmation: "Confirmación",
      deleteConfirmation: "¿Está seguro de que desea eliminar este elemento?",
      
      // Messages
      productSaved: "Producto guardado exitosamente",
      productDeleted: "Producto eliminado exitosamente",
      categorySaved: "Categoría guardada exitosamente",
      categoryDeleted: "Categoría eliminada exitosamente",
      subcategorySaved: "Subcategoría guardada exitosamente",
      subcategoryDeleted: "Subcategoría eliminada exitosamente",
      inquiryUpdated: "Consulta actualizada exitosamente",
      imageSaved: "Imagen guardada exitosamente",
      imageDeleted: "Imagen eliminada exitosamente",
    },
    
    // Legal page
    legal: {
      title: "Información Legal",
      subtitle: "Términos Legales y Política de Privacidad",
      impressum: "Información Legal",
      privacy: "Privacidad",
      impressumDescription: "Información legal sobre nuestra empresa",
      privacyDescription: "Cómo procesamos sus datos personales",
      company: "Empresa",
      address: "Dirección",
      contact: "Contacto",
      businessLicense: "Licencia Comercial",
      responsiblePerson: "Persona Responsable",
      responsiblePersonName: "Gerente",
      position: "Posición",
      website: "Sitio Web",
      websiteDescription: "Sitio web oficial de Excalibur Power Solutions",
      disclaimer: "Exención de Responsabilidad",
      disclaimerText: "Toda información sin garantía. Sujeto a cambios y errores.",
      dataCollection: "Recopilación de Datos",
      dataCollectionText: "Recopilamos los siguientes datos:",
      dataCollectionItem1: "Dirección IP e información del navegador",
      dataCollectionItem2: "Datos del formulario de contacto",
      dataCollectionItem3: "Estadísticas de uso del sitio web",
      dataCollectionItem4: "Cookies para mejorar la experiencia del usuario",
      dataUsage: "Uso de Datos",
      dataUsageText: "Utilizamos sus datos para:",
      dataUsageItem1: "Procesar sus consultas",
      dataUsageItem2: "Mejorar nuestros servicios",
      dataUsageItem3: "Análisis y optimización del sitio web",
      dataUsageItem4: "Comunicación con usted",
      cookies: "Cookies",
      cookiesText: "Utilizamos cookies para mejorar su experiencia y recopilar estadísticas anónimas.",
      analytics: "Análisis",
      analyticsText: "Recopilamos datos de uso anónimos para mejorar nuestro sitio web.",
      dataProtection: "Protección de Datos",
      dataProtectionText: "Sus datos se almacenan de forma segura y no se comparten con terceros.",
      userRights: "Sus Derechos",
      userRightsText: "Usted tiene derecho a:",
      userRightsItem1: "Información sobre sus datos almacenados",
      userRightsItem2: "Corrección de datos incorrectos",
      userRightsItem3: "Eliminación de sus datos",
      userRightsItem4: "Oponerse al procesamiento de datos",
      dataRetention: "Almacenamiento de Datos",
      dataRetentionText: "Almacenamos sus datos solo el tiempo necesario.",
      thirdParty: "Terceros",
      thirdPartyText: "No compartimos sus datos con terceros.",
      changes: "Cambios",
      changesText: "Nos reservamos el derecho de cambiar esta política de privacidad.",
      contactForPrivacy: "Contacto para Privacidad",
      contactForPrivacyText: "Para preguntas sobre privacidad, contáctenos:",
      lastUpdated: "Última Actualización",
    },
    


  },
  
  de: {
    // Offline and Cookie Banner
    offline: {
      message: 'Keine Internetverbindung'
    },
    cookieBanner: {
      title: 'Cookie-Verwendung',
      description: 'Wir verwenden Cookies, um Ihre Erfahrung auf unserer Website zu verbessern.',
      acceptAll: 'Alle akzeptieren',
      rejectAll: 'Alle ablehnen',
      settings: 'Einstellungen',
      settingsTitle: 'Cookie-Einstellungen',
      necessary: 'Notwendige Cookies',
      necessaryDesc: 'Diese Cookies sind für die Grundfunktionen der Website erforderlich.',
      analytics: 'Analyse-Cookies',
      analyticsDesc: 'Helfen uns zu verstehen, wie Sie unsere Website nutzen.',
      marketing: 'Marketing-Cookies',
      marketingDesc: 'Werden verwendet, um relevante Werbung anzuzeigen.',
      acceptSelected: 'Auswahl akzeptieren',
      required: 'Erforderlich'
    },
    
    // Navigation
    home: "Startseite",
    products: "Produkte",
    about: "Über uns",
    contact: "Kontakt",
    searchProducts: "Produkte suchen...",
    
    // Hero Section
    heroTitle1: "SOLARENERGIE FÜR KUBA",
    heroSubtitle1: "Hochwertige Solarkomponenten und Generatoren zum besten Preis",
    heroTitle2: "EXCALIBUR GENERATOREN",
    heroSubtitle2: "Leise Diesel- und Benzingeneratoren 2-20KVA",
    heroTitle3: "KOMPLETTLÖSUNGEN",
    heroSubtitle3: "Individuelle Solarsysteme nach Ihren Bedürfnissen",
    viewProducts: "Produkte ansehen",
    inquireNow: "Jetzt anfragen",
    customizeSystem: "System anpassen",
    
    // Features
    bestQuality: "Beste Qualität zum besten Preis",
    bestQualityDesc: "International zertifizierte Produkte zum besten Preis",
    fastDelivery: "Schnelle Verfügbarkeit",
    fastDeliveryDesc: "Schnelle Verfügbarkeit bei Selbstabholung - Habana del Este",
    
    // Products
    ourProducts: "UNSERE PRODUKTE",
    productsSubtitle: "Großangebot zertifizierter Solar-Komponenten, komplette Solarsysteme, Generatoren, Baumaterial und sonstige Materialien und vieles mehr.",
    completeSolarSystems: "Komplette Solarsysteme",
    completeSolarSystemsDesc: "Komplette Solaranlagen von 3 bis 30KW mit hochwertigen Komponenten zum besten Preis",
    solarPanels: "Solarpaneele",
    solarPanelsDesc: "Photovoltaik-Module monokristallin und polykristallin von 300W bis 550W mit IEC-Zertifizierung und Garantie. Höchste Effizienz für das tropische Klima Kubas.",
    inverters: "Wechselrichter",
    invertersDesc: "Hybrid- und Netz-Wechselrichter von 1KW bis 20KW mit integriertem MPPT, kompatibel mit Lithium-Batterien. Deutsche Technologie mit erweiterten Schutzfunktionen.",
    batteries: "Batterien",
    batteriesDesc: "LiFePO4-Lithium- und AGM-Tiefzyklus-Batterien von 100Ah bis 1000Ah. Entwickelt für Solarsysteme mit über 6000 Lebenszyklen.",

    accessories: "Zubehör",
    accessoriesDesc: "Solar-Einzelkomponenten: Bei uns bekommen Sie alles um jede Solar-Anlage ganz nach Bedarf und Wünschen, individuell zusammengestellt",
    viewSystems: "Systeme ansehen →",
    viewPanels: "Panels ansehen →",
    viewInverters: "Wechselrichter ansehen →",
    viewBatteries: "Batterien ansehen →",
    viewGenerators: "Generatoren ansehen →",
    viewAccessories: "Zubehör ansehen →",
    constructionMachines: "Baumaterial und sonstige Materialien",
    constructionMachinesDesc: "Wir bieten Baumaterial und sonstige Materialien für alle Projekte in Cuba. Hochwertige Technologie für professionelle Anwendungen.",
    
    // About - Neue Texte aus Kunden-PDF
    aboutTitle: "ÜBER UNS - EXCALIBUR POWER CUBA",
    aboutText1: "Wir sind eine KKMU aus Matanzas und vertreten die Excalibur Power Group in Kuba in Zusammenarbeit mit unserem deutschen Sponsor und Lieferanten. Excalibur gehört zu den weltweit führenden Herstellern von Generatoren, Solarenergie und vielen weiteren Komponenten.",
    aboutText2: "Wir garantieren beste Qualität zum besten Preis Dank unseres deutschen Joint-Venture-Partners, Sponsors und Lieferanten mit 35 Jahren Markterfahrung. Unser Lager befindet sich in Havanna del Este ist verkehrstechnisch günstig gelegen, um ganz Kuba mit schnellen Lieferungen und einem großen Lagerbestand zu beliefern. Unser Lieferant arbeitet direkt mit Fabriken zusammen, um beste Preise und höchste Qualität zu garantieren.",
    slogan: "Für eine hellere Zukunft in Cuba",
    yearsExperience: "Jahre Erfahrung",
    solarSystems: "Solarsysteme",
    ourWarehouse: "Unser Lager in Kuba",
    warehouseDesc: "Unser Lager befindet sich in Havanna Ost und ist Verkehrstechnisch günstig gelegen, und ist 24/7 besetzt, um für Kunden aus ganz Kuba gut erreichbar zu sein. Wir verfügen über einen reichhaltigen Lagerbestand. Unser Lieferant arbeitet direkt mit den Herstellern in aller Welt zusammen, um immer beste Preise und höchste Qualität gewährleisten zu können. Unsere Produkte haben eine Bestpreisgarantie und der Lagerbestand ist enorm, da unser Lieferant alles in großen Mengen einkauft und permanent volle Container aus Europa und Asien nach Kuba versendet.",
    
    // Customer Satisfaction - Neue Sektion
    customerSatisfactionTitle: "Kundenzufriedenheit",
    customerSatisfactionDesc: "Kundenzufriedenheit wird bei uns großgeschrieben und umfasst nicht nur beste Qualität zum besten Preis, sondern auch einen technischen 24/7 Support, der in Kuba einmalig ist. Unter +49 160 323 9439 erhalten Sie, auf alle Ihre Fragen eine kompetente Antwort.",
    jointVenturePartnerTitle: "Unser Joint Venture Partner und Lieferant",
    jointVenturePartnerSubtitle: "AFDL IMPORT & EXPORT GMBH & CO.KG",
    jointVenturePartnerDesc: "Mit 35 Jahren Erfahrung auf den internationalen Märkten und Sitz in Deutschland ermöglicht unser Joint Venture Partner den Import großer Produktmengen zu günstigen Preisen, sodass wir diese Waren wiederum günstig an unsere kubanische Kunden weitergeben können. Wir haben in Deutschland gelernt wie Markwirtschaft funktioniert.",
    
    // Additional About translations
    ourHistory: "Unsere Geschichte",
    satisfiedClients: "Zufriedene Kunden",
    technicalSupportStats: "Technischer Support",
    guaranteedQuality: "Beste Qualität zum besten Preis",
    guaranteedQualityDesc: "International zertifizierte Produkte mit erweiterten Garantien",
    expertTeam: "Expertenteam",
    
    // New German content
    illuminateCuba: "Für eine hellere Zukunft in Cuba",
    illuminateCubaDesc: "Wir erhellen Cuba besser",
    youngCubanCompany: "Junges kubanisches Unternehmen",
    youngCubanCompanyDesc: "Mit 35 Jahren Erfahrung unseres deutschen Partners",
    germanyPartnership: "35 Jahre Erfahrung mit Deutschland",
    germanyPartnershipDesc: "Professionelle Beratung und Unterstützung",
    experienceInSolar: "Erfahrung in der Solarenergie",
    experienceInSolarDesc: "Bewährte Technologie für die tropischen Bedingungen",
    languagesSpoken: "Sprachen",
    languagesSpokenDesc: "Wir sprechen: Spanisch, Englisch, Deutsch",

    pickupDelivery: "Pickup nur in Havanna del Este",
    pickupDeliveryDesc: "Keine Lieferung, Abholung erforderlich",
    supportLanguages: "Unterstützte Sprachen",
    supportLanguagesDesc: "Spanisch, Englisch, Deutsch",
    youngestCubanCompany: "Jüngstes kubanisches Unternehmen",
    youngestCubanCompanyDesc: "35 Jahre Erfahrung im Joint Venture mit Deutschland",
    
    // Contact information updates
    technicalAdvisory: "Technische Beratung",
    technicalAdvisoryPhone: "+49 160 323 9439",
    sales: "Verkauf",
    salesLabel: "Verkauf",
    salesPhone: "+53 5878 1416",
    deliveryPhone: "+53 54 73 14 90",
    germany: "Deutschland",
    international: "International",
    cuba: "Cuba",
    administration: "Administration",
    matanzas: "Matanzas",
    havanaDelEste: "Havanna del Este",
    contactDesc: "Kontaktieren Sie uns für professionelle Beratung und Unterstützung",
    expertTeamDesc: "Spezialisierte Techniker mit über 10 Jahren Erfahrung",
    bestPrice: "Bester Preis",
    bestPriceDesc: "Direkteinkäufe von Fabriken für beste Preise",
    internationalReach: "Internationale Reichweite",
    internationalReachDesc: "Unterstützt durch AFDL Import & Export aus Deutschland",
    whyChooseUs: "Warum Excalibur Cuba wählen?",
    strategicPartner: "Unser strategischer Partner",
    partnershipDesc: "Mit 35 Jahren Erfahrung auf den internationalen Märkten und Sitz in Deutschland ermöglicht unser Joint Venture Partner den Import großer Produktmengen zu günstigen Preisen, sodass wir diese Waren wiederum günstig an unsere kubanischen Kunden weitergeben können. Wir haben in Deutschland gelernt wie Marktwirtschaft funktioniert.",
    customerSatisfaction: "Kundenzufriedenheit",
    countriesServed: "Bediente Länder",
    containersSent: "Versendete Container",
    warehouseLocation: "Unser Lager in Havanna del Este ist strategisch gelegen, um ganz Kuba mit schnellen Lieferungen und reichlich Lagerbestand zu bedienen. Wir arbeiten direkt mit den Fabriken zusammen, um die besten Preise und höchste Qualität zu gewährleisten.",
    heroSubtitle: "Wir sind eine KKMU aus Matanzas und vertreten die Excalibur Power Group in Kuba in Zusammenarbeit mit unserem deutschen Sponsor und Lieferanten.",
    
    // Contact
    contactUs: "KONTAKTIEREN SIE UNS",
    contactSubtitle: "Wir konfigurieren das perfekte Solarsystem für Ihre Bedürfnisse",
    contactInfo: "Kontaktinformationen",
    whatsappAvailable: "WhatsApp verfügbar",
    supportSales: "Support+Verkauf",
    administrationSales: "Administration + Verkauf",
    deliveryPickup: "Lieferung / Abholung",
    warehouseDelivery: "Lager & Lieferung",
    emailAddresses: "E-Mail-Adressen",
    requestQuote: "Angebot anfordern",
    name: "Name",
    phone: "WhatsApp Nummer",
    email: "E-Mail",
    systemType: "Systemtyp",
    select: "Auswählen...",
    completeSolarSystem: "Komplettes Solarsystem",
    generatorOnly: "Nur Generator",
    individualComponents: "Einzelkomponenten",
    message: "Nachricht",
    messagePlaceholder: "Beschreiben Sie Ihren Energiebedarf...",
    sendRequest: "Anfrage senden",

    // Product Detail Page
    currentLanguage: "de",
    
    // Admin Dashboard Top-Level Keys
    dashboard: "Excalibur Cuba Admin",
    welcome: "Willkommen",
    analytics: "Analytics",
    websiteImages: "Website-Bilder",
    imageManager: "Bildverwaltung",
    viewWebsite: "Website ansehen",
    categories: "Kategorien",
    activeCategories: "Aktive Kategorien",
    activeProducts: "Aktive Produkte",
    inquiries: "Anfragen",
    newInquiries: "Neue Anfragen",
    settings: "Einstellungen",
    systemSettings: "System-Einstellungen",
    recentProducts: "Aktuelle Produkte",
    recentCategories: "Aktuelle Kategorien",
    recentInquiries: "Aktuelle Anfragen",
    addProduct: "Produkt hinzufügen",
    addCategory: "Kategorie hinzufügen",
    edit: "Bearbeiten",
    delete: "Löschen",
    view: "Ansehen",
    nameField: "Name",
    statusField: "Status",
    emailField: "E-Mail",
    messageField: "Nachricht",
    priceField: "Preis",
    categoryField: "Kategorie",
    active: "Aktiv",
    inactive: "Inaktiv",
    new: "Neu",
    replied: "Beantwortet",
    totalItems: "insgesamt",
    online: "Online",
    systemRunning: "System läuft",
    manageProducts: "Produkte verwalten",
    manageCategories: "Kategorien verwalten",
    customerInquiries: "Kundenanfragen",
    productManagement: "Produktverwaltung",
    productManagementDesc: "Verwalten Sie Ihre Produkte einfach und schnell",
    categoryManagement: "Kategorienverwaltung",
    categoryManagementDesc: "Organisieren Sie Ihre Produktkategorien",
    inquiryManagement: "Anfragenverwaltung",
    inquiryManagementDesc: "Bearbeiten Sie Kundenanfragen",
    noProductsAvailable: "Keine Produkte verfügbar",
    noCategoriesAvailable: "Keine Kategorien verfügbar",
    noInquiriesAvailable: "Keine Anfragen verfügbar",
    image: "Bild",
    stockStatus: "Lagerbestand",
    availability: "Verfügbarkeit",
    createdAt: "Erstellt am",
    actions: "Aktionen",
    
    productNotFound: "Produkt nicht gefunden",
    backToProducts: "Zurück zu Produkten",
    description: "Beschreibung",
    specifications: "Spezifikationen",
    inStock: "Auf Lager",
    limitedStock: "Begrenzter Lagerbestand",
    outOfStock: "Nicht verfügbar",
    interestedInProduct: "Interessiert an diesem Produkt?",
    contactWhatsApp: "Über WhatsApp kontaktieren",
    contactEmail: "E-Mail senden",
    contactPhone: "Anrufen",
    qualityGuarantee: "Qualitätsgarantie",
    expertSupport: "Expertenunterstützung",
    germanQuality: "Zertifizierte deutsche Qualität",
    monthlyContainerImports: "Monatliche Container-Importe",
    bestPrices: "Die besten Marktpreise",
    
    // SEO Content Section
    solarEnergyTitle: "Solarenergie in Kuba: Eine nachhaltige Lösung",
    solarEnergyDescription: "Kuba befindet sich in einer geografisch privilegierten Position für die Nutzung der Sonnenenergie, mit über 330 Sonnentagen pro Jahr und einer durchschnittlichen Sonneneinstrahlung von 5,5 kWh/m²/Tag. Diese reichliche Sonneneinstrahlung macht die Insel zu einem idealen Ort für die Implementierung von Photovoltaikanlagen.",
    solarAdvantagesTitle: "Vorteile der Solarenergie in Kuba",
    solarAdvantage1: "Werden Sie ihr eigener Stromproduzent",
    solarAdvantage2: "Machen Sie sich unabhängig vom öffentlichen Stromnetz und den damit verbundenen Problemen",
    solarAdvantage3: "Für ihr Geschäft und zu Hause",
    solarAdvantage4: "Freie Energie aus der Sonne",
    solarAdvantage5: "Beitrag zum Umweltschutz",
    ourSystemsTitle: "Unsere Solarsysteme",
    systemFeature1: "Komplette Systeme von 3KW bis 30KW",
    systemFeature2: "Hocheffiziente zertifizierte Solarmodule für die Montage an und auf Gebäuden",
    systemFeature3: "Hybrid-Wechselrichter mit Batteriespeicher",
    systemFeature4: "Langlebige LiFePO4-Batterien",
    systemFeature5: "Installation und spezialisierte technische Unterstützung",
    
    // Additional SEO Content
    generatorsTitle: "Backup-Generatoren: Garantierte Energiesicherheit",
    generatorsDescription: "Wir ergänzen unsere Solarsysteme mit modernen geräuscharmen Generatoren und bieten eine integrierte Energielösung, die kontinuierliche Stromversorgung gewährleistet. Unsere Diesel- und Benzin-Generatoren bieten 2KVA bis 10KVA, ideal für Haushalte, kleine Unternehmen und industrielle Anwendungen.",
    whyChooseTitle: "Warum Excalibur Power Cuba wählen?",
    whyChooseDescription: "Mit 35 Jahren Erfahrung unseres deutschen Partners AFDL IMPORT & EXPORT bieten wir das beste Preis-Leistungs-Verhältnis für Solarkomponenten und Generatoren. Unser Engagement ist es, Kuba mit modernster Technologie und professionellem Service wieder zu erleuchten.",
    monthlyImportsTitle: "Permanente Lieferungen",
    monthlyImportsDescription: "Wir erhalten permanente Container-Lieferungen aus Deutschland und der ganzen Welt mit Produkten und der neuesten Markttechnologien. Unser Lager in Havanna del Este hält ein umfangreichen Bestand für Sie bereit, um die sofortige Nachfrage unserer Kunden zu erfüllen. Dies ermöglicht uns, wettbewerbsfähige Preise und eine sofortige oder zeitnahe Verfügbarkeit anzubieten.",
    
    // Footer
    quickLinks: "Schnellzugriff",
    allRightsReserved: "Alle Rechte vorbehalten.",
    companyDescription: "Professioneller Importeur von Solarkomponenten und Generatoren in Kuba. Offizielle Vertreter der Excalibur Power Group.",
    
    // WhatsApp
    needHelp: "Brauchen Sie Hilfe?",
    whatsapp: "WhatsApp",
    
    // Form Messages
    formSubmitted: "Formular gesendet. Wir werden uns bald bei Ihnen melden.",
    formError: "Fehler beim Senden des Formulars. Bitte versuchen Sie es erneut.",
    
    // Products Page
    selectCategory: "Kategorie auswählen",
    allCategories: "Alle Kategorien",
    noProductsFound: "Keine Produkte gefunden",
    noProductsMessage: "Keine Produkte in dieser Kategorie verfügbar.",
    viewDetails: "Details anzeigen",
    subcategories: "Unterkategorien",
    subcategory: "Unterkategorie",
    category: "Kategorie",
    product: "Produkt",
    moreProducts: "Weitere Produkte",
    noContentFound: "Keine Inhalte gefunden",
    noCategoryContent: "Diese Kategorie enthält aktuell keine Unterkategorien oder Produkte.",
    productsMetaDesc: "Excalibur Cuba Produkte: Komplette Solarsysteme, Generatoren, Lithium-Batterien, Wechselrichter, MC4-Kabel, Bagger, Kompressoren, Wasserfilter, Pizzaöfen in Matanzas.",
    productsMetaKeywords: "Excalibur Cuba Produkte, Solarsysteme Kuba, Generatoren Kuba, Lithium-Batterien Kuba, Wechselrichter Kuba, MC4-Kabel Kuba, Bagger Kuba, Kompressoren Kuba, Matanzas",
    featured: "Empfohlen",
    in_stock: "Vorrätig",
    limited: "Begrenzt verfügbar",
    out_of_stock: "Unterwegs",
    onTheWay: "Unterwegs",
    seaTransport: "Auf dem Seeweg",
    priceOnRequest: "Preis auf Anfrage",
    onSale: "Im Angebot",
    wattPeak: "Watt Peak",
    needCustomSolution: "Benötigen Sie eine maßgeschneiderte Lösung?",
    customSolutionDescription: "Unsere Experten können ein spezifisches Solarsystem für Ihre Energiebedürfnisse entwerfen.",
    
    // Product Detail Extensions
    productFeatures: "Produkteigenschaften",
    qualityAssurance: "Qualitätssicherung",
    qualityAssuranceDesc: "International zertifizierte Produkte mit erweiterten Garantien",
    highPerformance: "Hohe Leistung",
    highPerformanceDesc: "Modernste Technologie für maximale Energieeffizienz",
    warrantyDesc: "Internationale erweiterte Garantien",
    installationSetup: "Installation und Einrichtung",
    professionalInstallation: "Professionelle Installation",
    professionalInstallationDesc: "Unser Spezialistenteam übernimmt die komplette Installation Ihres Solarsystems.",
    installationStep1: "Technische Standortbewertung und Systemdimensionierung",
    installationStep2: "Professionelle Installation von Panels, Wechselrichter und Batterien",
    installationStep3: "Konfiguration und Inbetriebnahme des kompletten Systems",
    installationStep4: "Benutzerschulung und Übergabe der Dokumentation",
    technicalSupportExtended: "Spezialisierter technischer Support rund um die Uhr verfügbar.",
    supportChannels: "Support-Kanäle",
    technicalDetails: "Technische Details",
    certifications: "Zertifizierungen",
    certified: "Zertifiziert",
    ceCompliance: "CE-Konformität",
    iecStandard: "IEC 61215 Standard",
    rohs: "RoHS-Konformität",
    iso9001: "ISO 9001:2015",
    warranty: "Garantie",
    manufacturerWarranty: "Herstellergarantie",
    manufacturerWarrantyDesc: "Erweiterte Produktgarantie gegen Herstellungsfehler",
    performanceWarranty: "Leistungsgarantie",
    performanceWarrantyDesc: "Garantie für mindestens 80% Leistung",
    supportWarranty: "Support-Garantie",
    supportWarrantyDesc: "Kostenloser technischer Support während der gesamten Produktlebensdauer",
    aboutExcaliburCuba: "Über Excalibur Cuba",
    aboutExcaliburDesc: "Offizielle Vertreter der Excalibur Power Group in Kuba, die hochwertige Energielösungen anbieten, um Kubas Zukunft zu erhellen.",
    
    // Missing keys for Home page
    ourServices: "Unsere Leistungen für Sie",
    warehouse: "Abholung / Lager",
    havanaLocation: "Havanna del Este",
    emailLabel: "E-Mail Kontakt:",
    
    // Home page boxes
    experience: "35 Jahre Erfahrung",
    experienceDesc: "Unser deutscher Lieferant AFDL verfügt über 35 Jahre internationaler Erfahrung",
    directImport: "Direkt-Importe aus aller Welt nach Cuba",
    directImportDesc: "Permanente Lieferungen hochwertiger Solarkomponenten und Generatoren aus Deutschland und weltweiten Fertigungsstandorten",
    homeLocation: "Verkehrsgünstige zentrale Lage",
    homeLocationDesc: "Lager in Havanna del Este für schnelle Verfügbarkeit bei Selbstabholung",
    
    // Services
    components: "Solar-Einzelkomponenten",
    solarSystemsDesc: "Komplette Solaranlagen von 3 bis 30KW mit hochwertigen Komponenten zum besten Preis",
    componentsDesc: "Bei uns bekommen Sie alles um jede Solar-Anlage ganz nach Bedarf und Wünschen, individuell zusammengestellt",
    
    // Cart System
    cart: "Warenkorb",
    addToCart: "In den Warenkorb",
    viewCart: "Warenkorb anzeigen",
    cartEmpty: "Ihr Warenkorb ist leer",
    cartEmptyDesc: "Fügen Sie Produkte hinzu, um fortzufahren",
    continueWhatsApp: "Mit WhatsApp fortfahren",
    removeFromCart: "Entfernen",
    clearCart: "Alles löschen",
    quantity: "Menge",
    total: "Gesamt",
    subtotal: "Zwischensumme",
    cartItems: "Artikel",
    myCart: "Mein Warenkorb",
    sendViaWhatsApp: "Über WhatsApp senden",
    whatsAppMessage: "Hallo, ich interessiere mich für diese Produkte:",
    
    // Mobile Image Gallery
    showMobileImages: "Alle Bilder anzeigen",
    hideMobileImages: "Bilder ausblenden",

    
    // Product page translations
    productCount: "Produkte",
    backToCategories: "Zurück zu Kategorien",
    selectSubcategory: "Wählen Sie eine Unterkategorie aus",
    inThisCategory: "in dieser Kategorie",
    
    // Contact page
    contactLocation: "Standort",
    contactLocationTitle: "Unser Standort",
    contactLocationDesc: "Hauptlager in Havanna del Este mit einfachem Zugang von ganz Kuba",
    
    // Admin Backend
    admin: {
      // Login
      loginTitle: "Anmelden",
      loginSubtitle: "Zugang zum Verwaltungsbereich",
      username: "Benutzername",
      password: "Passwort",
      loginButton: "Anmelden",
      loginError: "Falscher Benutzername oder Passwort",
      
      // Dashboard
      dashboard: "Dashboard",
      overview: "Übersicht",
      totalProducts: "Gesamte Produkte",
      totalCategories: "Gesamte Kategorien", 
      pendingInquiries: "Ausstehende Anfragen",
      activeProducts: "Aktive Produkte",
      recentProducts: "Neueste Produkte",
      recentCategories: "Neueste Kategorien",
      recentInquiries: "Neueste Anfragen",
      viewAll: "Alle anzeigen",
      
      // Products
      products: "Produkte",
      productManagement: "Produktverwaltung",
      currentProducts: "Aktuelle Produkte",
      newProduct: "Neues Produkt",
      addProduct: "Produkt hinzufügen",
      editProduct: "Produkt bearbeiten",
      productName: "Produktname",
      productDescription: "Beschreibung",
      productSpecifications: "Spezifikationen",
      productPrice: "Preis",
      productStock: "Verfügbarkeit",
      productImages: "Bilder",
      productCategory: "Kategorie",
      productSubcategory: "Unterkategorie",
      selectCategory: "Kategorie auswählen",
      selectSubcategory: "Unterkategorie auswählen",
      inStock: "Auf Lager",
      limitedStock: "Begrenzter Lagerbestand",
      outOfStock: "Nicht verfügbar",
      saveProduct: "Produkt speichern",
      
      // Categories
      categories: "Kategorien",
      addCategory: "Kategorie hinzufügen",
      editCategory: "Kategorie bearbeiten",
      categoryName: "Kategoriename",
      categoryDescription: "Beschreibung",
      categoryImage: "Bild",
      saveCategory: "Kategorie speichern",
      
      // Subcategories  
      subcategories: "Unterkategorien",
      addSubcategory: "Unterkategorie hinzufügen",
      editSubcategory: "Unterkategorie bearbeiten",
      subcategoryName: "Unterkategoriename",
      parentCategory: "Hauptkategorie",
      saveSubcategory: "Unterkategorie speichern",
      
      // Inquiries
      inquiries: "Anfragen",
      inquiryName: "Name",
      inquiryEmail: "E-Mail",
      inquiryPhone: "Telefon",
      inquirySubject: "Betreff",
      inquiryMessage: "Nachricht",
      inquiryStatus: "Status",
      inquiryDate: "Datum",
      statusNew: "Neu",
      statusInProgress: "In Bearbeitung",
      statusCompleted: "Abgeschlossen",
      
      // Images
      images: "Bilder",
      uploadImages: "Bilder hochladen",
      selectImage: "Bild auswählen",
      imageGallery: "Bildergalerie",
      
      // Analytics
      analytics: "Statistiken",
      totalViews: "Gesamte Aufrufe",
      uniqueVisitors: "Eindeutige Besucher",
      topPages: "Top Seiten",
      topCountries: "Top Länder",
      
      // Common
      actions: "Aktionen",
      edit: "Bearbeiten",
      delete: "Löschen",
      cancel: "Abbrechen",
      save: "Speichern",
      back: "Zurück",
      backToDashboard: "Zurück zum Dashboard",
      search: "Suchen",
      filter: "Filtern",
      all: "Alle",
      yes: "Ja",
      no: "Nein",
      loading: "Lade...",
      success: "Erfolg",
      error: "Fehler",
      confirmation: "Bestätigung",
      deleteConfirmation: "Sind Sie sicher, dass Sie dieses Element löschen möchten?",
      
      // Messages
      productSaved: "Produkt erfolgreich gespeichert",
      productDeleted: "Produkt erfolgreich gelöscht",
      categorySaved: "Kategorie erfolgreich gespeichert",
      categoryDeleted: "Kategorie erfolgreich gelöscht",
      subcategorySaved: "Unterkategorie erfolgreich gespeichert",
      subcategoryDeleted: "Unterkategorie erfolgreich gelöscht",
      inquiryUpdated: "Anfrage erfolgreich aktualisiert",
      imageSaved: "Bild erfolgreich gespeichert",
      imageDeleted: "Bild erfolgreich gelöscht",
    },
    
    // Legal page
    legal: {
      title: "Rechtliche Informationen",
      subtitle: "Impressum und Datenschutzerklärung",
      impressum: "Impressum",
      privacy: "Datenschutz",
      impressumDescription: "Rechtliche Informationen über unser Unternehmen",
      privacyDescription: "Wie wir Ihre persönlichen Daten verarbeiten",
      company: "Unternehmen",
      address: "Adresse",
      contact: "Kontakt",
      businessLicense: "Geschäftslizenz",
      responsiblePerson: "Verantwortliche Person",
      responsiblePersonName: "Verantwortlicher",
      position: "Position",
      website: "Website",
      websiteDescription: "Offizielle Website von Excalibur Power Solutions",
      disclaimer: "Haftungsausschluss",
      disclaimerText: "Alle Angaben ohne Gewähr. Änderungen und Irrtümer vorbehalten.",
      dataCollection: "Datenerfassung",
      dataCollectionText: "Wir erfassen folgende Daten:",
      dataCollectionItem1: "IP-Adresse und Browser-Informationen",
      dataCollectionItem2: "Kontaktformular-Daten",
      dataCollectionItem3: "Website-Nutzungsstatistiken",
      dataCollectionItem4: "Cookies für bessere Benutzererfahrung",
      dataUsage: "Datenverwendung",
      dataUsageText: "Wir verwenden Ihre Daten für:",
      dataUsageItem1: "Bearbeitung Ihrer Anfragen",
      dataUsageItem2: "Verbesserung unserer Dienstleistungen",
      dataUsageItem3: "Website-Analyse und -Optimierung",
      dataUsageItem4: "Kommunikation mit Ihnen",
      cookies: "Cookies",
      cookiesText: "Wir verwenden Cookies, um Ihre Erfahrung zu verbessern und anonyme Statistiken zu sammeln.",
      analytics: "Analytik",
      analyticsText: "Wir sammeln anonyme Nutzungsdaten, um unsere Website zu verbessern.",
      dataProtection: "Datenschutz",
      dataProtectionText: "Ihre Daten werden sicher gespeichert und nicht an Dritte weitergegeben.",
      userRights: "Ihre Rechte",
      userRightsText: "Sie haben das Recht auf:",
      userRightsItem1: "Auskunft über Ihre gespeicherten Daten",
      userRightsItem2: "Berichtigung unrichtiger Daten",
      userRightsItem3: "Löschung Ihrer Daten",
      userRightsItem4: "Widerspruch gegen die Datenverarbeitung",
      dataRetention: "Datenspeicherung",
      dataRetentionText: "Wir speichern Ihre Daten nur so lange wie nötig.",
      thirdParty: "Drittanbieter",
      thirdPartyText: "Wir geben Ihre Daten nicht an Dritte weiter.",
      changes: "Änderungen",
      changesText: "Wir behalten uns das Recht vor, diese Datenschutzerklärung zu ändern.",
      contactForPrivacy: "Kontakt für Datenschutz",
      contactForPrivacyText: "Bei Fragen zum Datenschutz kontaktieren Sie uns:",
      lastUpdated: "Letzte Aktualisierung",
    }
  },
  
  en: {
    // Offline
    offline: {
      message: 'No internet connection'
    },

    // Navigation
    home: "Home",
    products: "Products",
    about: "About Us",
    contact: "Contact",
    searchProducts: "Search products...",
    
    // Hero Section
    heroTitle1: "SOLAR ENERGY FOR CUBA",
    heroSubtitle1: "High-quality solar components and generators at the best price",
    heroTitle2: "EXCALIBUR GENERATORS",
    heroSubtitle2: "Silent diesel and gasoline generators 2-20KVA",
    heroTitle3: "COMPLETE SOLUTIONS",
    heroSubtitle3: "Custom solar systems according to your needs",
    viewProducts: "View Products",
    inquireNow: "Inquire Now",
    customizeSystem: "Customize System",
    
    // Features
    bestQuality: "Best Quality at Best Price",
    bestQualityDesc: "Internationally certified products at best price",
    fastDelivery: "Fast Pickup",
    fastDeliveryDesc: "Fast pickup availability - Habana del Este",
    
    // Products
    ourProducts: "OUR PRODUCTS",
    productsSubtitle: "Large selection of certified solar components, complete solar systems, generators, construction materials and other materials and much more.",
    completeSolarSystems: "Complete Solar Systems",
    completeSolarSystemsDesc: "Pre-configured systems from 1KW to 20KW with hybrid inverter, monocrystalline panels and options with/without battery. Ideal for homes and businesses in Cuba with international warranty.",
    solarPanels: "Solar Panels",
    solarPanelsDesc: "Photovoltaic modules monocrystalline and polycrystalline from 300W to 550W with IEC certification and warranty. Maximum efficiency for Cuba's tropical climate.",
    inverters: "Inverters",
    invertersDesc: "Hybrid and grid-tie inverters from 1KW to 20KW with integrated MPPT, compatible with lithium batteries. German technology with advanced protections.",
    batteries: "Batteries",
    batteriesDesc: "LiFePO4 lithium and AGM deep cycle batteries from 100Ah to 1000Ah. Designed for solar systems with over 6000 life cycles.",
    generators: "Generators",
    generatorsDesc: "Silent Excalibur diesel and gasoline generators from 2KVA to 20KVA with electric start, low consumption and noise level below 60dB.",
    accessories: "Accessories",
    accessoriesDesc: "Complete installation kit: aluminum structures, solar cables, fuses, MC4 connectors, grounding systems and specialized tools.",
    viewSystems: "View Systems →",
    viewPanels: "View Panels →",
    viewInverters: "View Inverters →",
    viewBatteries: "View Batteries →",
    viewGenerators: "View Generators →",
    viewAccessories: "View Accessories →",
    
    // About - Neue englische Übersetzung  
    aboutTitle: "ABOUT US - EXCALIBUR POWER CUBA",
    aboutText1: "We are an SME from Matanzas and represent the Excalibur Power Group in Cuba in collaboration with our German sponsor and supplier. Excalibur is among the world's leading manufacturers of generators, solar energy, and many other components.",
    aboutText2: "We guarantee the best quality at the best price thanks to our German Joint Venture partner, sponsor and supplier with 35 years of market experience. Our warehouse is located in Havana del Este and is favorably positioned for traffic to supply all of Cuba with fast deliveries and a large warehouse stock. Our supplier works directly with factories to guarantee the best prices and highest quality.",
    yearsExperience: "Years of experience",
    solarSystems: "Solar systems",
    ourWarehouse: "Our Warehouse in Cuba",
    warehouseDesc: "Our warehouse is located in Havana East and is conveniently located for traffic, and is staffed 24/7, to be easily accessible for customers from all over Cuba. We have an abundant warehouse stock. Our supplier works directly with manufacturers all over the world to always ensure the best prices and highest quality. Our products have a best price guarantee and the warehouse stock is enormous, as our supplier buys everything in large quantities and permanently sends full containers from Europe and Asia to Cuba.",
    
    // Customer Satisfaction - New Section
    customerSatisfactionTitle: "Customer Satisfaction",
    customerSatisfactionDesc: "Customer satisfaction is very important to us and includes not only the best quality at the best price, but also a technical 24/7 support that is unique in Cuba. At +49 160 323 9439 you receive a competent answer to all your questions.",
    jointVenturePartnerTitle: "Our Joint Venture Partner and Supplier",
    jointVenturePartnerSubtitle: "AFDL IMPORT & EXPORT GMBH & CO.KG",
    jointVenturePartnerDesc: "With 35 years of experience in international markets and based in Germany, our Joint Venture partner enables the import of large product quantities at favorable prices, so we can pass these goods on to our Cuban customers at favorable prices. We have learned in Germany how market economy works.",
    
    // Additional About translations
    ourHistory: "Our History",
    satisfiedClients: "Satisfied Clients",
    technicalSupportStats: "Technical Support",
    guaranteedQuality: "Best quality at the best price",
    guaranteedQualityDesc: "Internationally certified products with extended warranties",
    expertTeam: "Expert Team",
    expertTeamDesc: "Specialized technicians with over 10 years of experience",
    bestPrice: "Best Price",
    bestPriceDesc: "Direct factory purchases for the best prices",
    internationalReach: "International Reach",
    internationalReachDesc: "Backed by AFDL Import & Export from Germany",
    whyChooseUs: "Why Choose Excalibur Cuba?",
    strategicPartner: "Our Strategic Partner",
    partnershipDesc: "With 35 years of experience in international markets and headquarters in Germany, our Joint Venture partner enables the import of large product quantities at favorable prices, so that we can pass these goods on to our Cuban customers at favorable prices. We have learned in Germany how market economy works.",
    customerSatisfaction: "Customer Satisfaction",
    countriesServed: "Countries Served",
    containersSent: "Containers Sent",
    warehouseLocation: "Our warehouse in Havanna del Este is strategically located to serve all of Cuba with fast deliveries and abundant stock. We work directly with factories to guarantee the best prices and highest quality.",
    heroSubtitle: "We are an SME from Matanzas and represent the Excalibur Power Group in Cuba in collaboration with our German sponsor and supplier.",
    
    // Contact
    contactUs: "CONTACT US",
    contactSubtitle: "We configure the perfect solar system for your needs",
    contactInfo: "Contact Information",
    technicalAdvisory: "Technical Advisory",
    whatsappAvailable: "WhatsApp available",
    administration: "Administration",
    warehouseDelivery: "Warehouse & Delivery",
    emailAddresses: "Email Addresses",
    requestQuote: "Request Quote",
    name: "Name",
    phone: "Phone Number",
    email: "Email",
    systemType: "System Type",
    select: "Select...",
    completeSolarSystem: "Complete Solar System",
    generatorOnly: "Generator Only",
    individualComponents: "Individual Components",
    message: "Message",
    messagePlaceholder: "Describe your energy needs...",
    sendRequest: "Send Request",
    
    // Product Detail Page
    currentLanguage: "en",
    productNotFound: "Product not found",
    backToProducts: "Back to products",
    description: "Description",
    specifications: "Specifications",
    inStock: "In Stock",
    limitedStock: "Limited Stock",
    outOfStock: "Out of Stock",
    interestedInProduct: "Interested in this product?",
    contactWhatsApp: "Contact via WhatsApp",
    contactEmail: "Send Email",
    contactPhone: "Call",
    qualityGuarantee: "Quality Guarantee",
    expertSupport: "Expert Support",
    germanQuality: "Certified German quality",
    monthlyContainerImports: "Monthly container imports",
    bestPrices: "The best market prices",
    
    // Footer
    quickLinks: "Quick Links",
    allRightsReserved: "All rights reserved.",
    companyDescription: "Professional importer of solar components and generators in Cuba. Official representatives of Excalibur Power Group.",
    
    // WhatsApp
    needHelp: "Need help?",
    whatsapp: "WhatsApp",
    
    // Form Messages
    formSubmitted: "Form submitted. We will contact you soon.",
    formError: "Error submitting form. Please try again.",
    
    // Additional SEO Content
    solarEnergyTitle: "Solar Energy in Cuba: A Sustainable Solution",
    solarEnergyDescription: "Cuba is in a geographically privileged position for solar energy utilization, with over 330 sunny days per year and an average solar irradiation of 5.5 kWh/m²/day. This abundant solar radiation makes the island an ideal location for photovoltaic system implementation.",
    solarAdvantagesTitle: "Advantages of Solar Energy in Cuba",
    solarAdvantage1: "Become your own electricity producer",
    solarAdvantage2: "Become independent from the public power grid and its associated problems",
    solarAdvantage3: "For your business and home",
    solarAdvantage4: "Free energy from the sun",
    solarAdvantage5: "Contributing to environmental protection",
    ourSystemsTitle: "Our Solar Systems",
    systemFeature1: "Complete systems from 3KW to 30KW",
    systemFeature2: "High-efficiency certified solar modules for mounting on and over buildings",
    systemFeature3: "Hybrid inverters with battery backup",
    systemFeature4: "Long-lasting LiFePO4 batteries",
    systemFeature5: "Installation and specialized technical support",
    generatorsTitle: "Backup Generators: Guaranteed Energy Security",
    generatorsDescription: "We complement our solar systems with modern silent generators, providing an integrated energy solution that ensures continuous power supply. Our diesel and gasoline generators offer 2KVA to 10KVA, ideal for homes, small businesses, and industrial applications.",
    whyChooseTitle: "Why Choose Excalibur Power Cuba?",
    whyChooseDescription: "We are a young company in Cuba. With the support of our German partner company AFDL IMPORT & EXPORT, with 35 years of international professional experience, we are able to deliver the best quality at the best price. We love our country Cuba and our commitment is to illuminate Cuba's future with cutting-edge technology and professional service.",
    monthlyImportsTitle: "Permanent Deliveries",
    monthlyImportsDescription: "We spare no costs or efforts, through permanent research and imports of innovative technologies from around the world, to contribute to having available the components that our customers and future customers will demand.",
    
    // Products Page
    selectCategory: "Select category",
    selectSubcategory: "Select a subcategory",
    backToCategories: "Back to Categories",
    allCategories: "All categories",
    noProductsFound: "No products found",
    noProductsMessage: "No products available in this category.",
    viewDetails: "View Details",
    subcategories: "Subcategories",
    subcategory: "Subcategory",
    category: "Category",
    product: "Product",
    moreProducts: "More Products",
    noContentFound: "No content found",
    noCategoryContent: "This category currently contains no subcategories or products.",
    productsMetaDesc: "Excalibur Cuba Products: Complete solar systems, generators, lithium batteries, inverters, MC4 cables, excavators, compressors, water filters, pizza ovens in Matanzas.",
    productsMetaKeywords: "Excalibur Cuba products, solar systems Cuba, generators Cuba, lithium batteries Cuba, inverters Cuba, MC4 cables Cuba, excavators Cuba, compressors Cuba, Matanzas",
    featured: "Featured",
    in_stock: "In stock",
    limited: "Limited stock",
    out_of_stock: "On the way",
    onTheWay: "On the way",
    seaTransport: "Sea transport",
    priceOnRequest: "Price on request",
    onSale: "On sale",
    wattPeak: "Watt Peak",
    needCustomSolution: "Need a custom solution?",
    customSolutionDescription: "Our experts can design a specific solar system for your energy needs.",
    
    // Offline and Cookie Banner
    cookieBanner: {
      title: 'Cookie Usage',
      description: 'We use cookies to improve your experience on our website.',
      acceptAll: 'Accept all',
      rejectAll: 'Reject all',
      settings: 'Settings',
      settingsTitle: 'Cookie Preferences',
      necessary: 'Necessary cookies',
      necessaryDesc: 'These cookies are essential for the website to function.',
      analytics: 'Analytics cookies',
      analyticsDesc: 'Help us understand how you use our website.',
      marketing: 'Marketing cookies',
      marketingDesc: 'Used to show relevant advertisements.',
      acceptSelected: 'Accept selected',
      required: 'Required'
    },
    
    // Product Detail Extensions
    productFeatures: "Product Features",
    qualityAssurance: "Quality Assurance",
    installationGuide: "Installation Guide",
    certifications: "Certifications",
    supportAndWarranty: "Support & Warranty",
    relatedProducts: "Related Products",
    productGallery: "Product Gallery",
    qualityAssuranceDesc: "Internationally certified products with extended warranties",
    highPerformance: "High Performance",
    highPerformanceDesc: "Cutting-edge technology for maximum energy efficiency",
    warrantyDesc: "Extended international warranties",
    installationSetup: "Installation & Setup",
    professionalInstallation: "Professional Installation",
    professionalInstallationDesc: "Our specialized team handles the complete installation of your solar system.",
    installationStep1: "Technical site assessment and system sizing",
    installationStep2: "Professional installation of panels, inverter and batteries",
    installationStep3: "Configuration and commissioning of the complete system",
    installationStep4: "User training and documentation handover",
    technicalSupportExtended: "Specialized technical support available 24 hours a day.",
    supportChannels: "Support Channels",
    technicalDetails: "Technical Details",
    certified: "Certified",
    ceCompliance: "CE Compliance",
    iecStandard: "IEC 61215 Standard",
    rohs: "RoHS Compliance",
    iso9001: "ISO 9001:2015",
    warranty: "Warranty",
    manufacturerWarranty: "Manufacturer Warranty",
    manufacturerWarrantyDesc: "Extended product warranty against manufacturing defects",
    performanceWarranty: "Performance Warranty",
    performanceWarrantyDesc: "Warranty for minimum 80% power output",
    supportWarranty: "Support Warranty",
    supportWarrantyDesc: "Free technical support throughout the product's lifespan",
    aboutExcaliburCuba: "About Excalibur Cuba",
    aboutExcalibirDesc: "Official representatives of Excalibur Power Group in Cuba, providing high-quality energy solutions to illuminate Cuba's future.",
    
    // Navigation and Categories
    productCount: "Products",
    inThisCategory: "in this category",
    
    // Missing keys for Home page
    ourServices: "Our Services",
    contactDesc: "We are happy to put together the right solar system for you according to your wishes",
    international: "International",
    salesLabel: "Sales",
    technicalAdvisoryPhone: "+49 160 323 9439",
    salesPhone: "+53 5878 1416", 
    deliveryPhone: "+53 54 73 14 90",
    cuba: "Cuba",
    warehouse: "Pickup / Warehouse",
    havanaLocation: "Havana del Este",
    emailLabel: "Email Contact:",
    
    // Home page boxes
    experience: "35 years of experience",
    experienceDesc: "Our German supplier AFDL has 35 years of international experience",
    directImport: "Direct imports from all over the world to Cuba",
    directImportDesc: "Permanent deliveries of high-quality solar components and generators from Germany and worldwide manufacturing centers",
    homeLocation: "Strategic central location",
    homeLocationDesc: "Warehouse in Havana del Este for fast availability with pickup",
    
    // Services
    components: "Individual Solar Components",
    solarSystemsDesc: "Complete solar systems from 3 to 30KW with high-quality components at the best price",
    componentsDesc: "With us you get everything to assemble any solar system according to needs and wishes, individually configured",
    
    // Cart System
    cart: "Cart",
    addToCart: "Add to Cart",
    viewCart: "View Cart",
    cartEmpty: "Your cart is empty",
    cartEmptyDesc: "Add products to your cart to continue",
    continueWhatsApp: "Continue with WhatsApp",
    removeFromCart: "Remove",
    clearCart: "Clear all",
    quantity: "Quantity",
    total: "Total",
    subtotal: "Subtotal",
    cartItems: "items",
    myCart: "My Cart",
    sendViaWhatsApp: "Send via WhatsApp",
    whatsAppMessage: "Hello, I'm interested in these products:",
    
    // Mobile Image Gallery
    showMobileImages: "View all images",
    hideMobileImages: "Hide images",
    
    constructionMachines: "Construction materials and other materials",
    constructionMachinesDesc: "High-quality construction materials for your projects",
    
    // Contact page
    contactLocation: "Location",
    contactLocationTitle: "Our Location",
    contactLocationDesc: "Main warehouse in Havana del Este with easy access from all of Cuba",
    
    // About
    slogan: "For a brighter future in Cuba",
    
    // Admin Backend
    admin: {
      // Login
      loginTitle: "Login",
      loginSubtitle: "Access the administration panel",
      username: "Username",
      password: "Password",
      loginButton: "Login",
      loginError: "Incorrect username or password",
      
      // Dashboard
      dashboard: "Dashboard",
      overview: "Overview",
      totalProducts: "Total Products",
      totalCategories: "Total Categories", 
      pendingInquiries: "Pending Inquiries",
      activeProducts: "Active Products",
      recentProducts: "Recent Products",
      recentCategories: "Recent Categories",
      recentInquiries: "Recent Inquiries",
      viewAll: "View All",
      
      // Products
      products: "Products",
      productManagement: "Product Management",
      currentProducts: "Current Products",
      newProduct: "New Product",
      addProduct: "Add Product",
      editProduct: "Edit Product",
      productName: "Product Name",
      productDescription: "Description",
      productSpecifications: "Specifications",
      productPrice: "Price",
      productStock: "Availability",
      productImages: "Images",
      productCategory: "Category",
      productSubcategory: "Subcategory",
      selectCategory: "Select category",
      selectSubcategory: "Select subcategory",
      inStock: "In Stock",
      limitedStock: "Limited Stock",
      outOfStock: "Out of Stock",
      saveProduct: "Save Product",
      
      // Categories
      categories: "Categories",
      addCategory: "Add Category",
      editCategory: "Edit Category",
      categoryName: "Category Name",
      categoryDescription: "Description",
      categoryImage: "Image",
      saveCategory: "Save Category",
      
      // Subcategories  
      subcategories: "Subcategories",
      addSubcategory: "Add Subcategory",
      editSubcategory: "Edit Subcategory",
      subcategoryName: "Subcategory Name",
      parentCategory: "Parent Category",
      saveSubcategory: "Save Subcategory",
      
      // Inquiries
      inquiries: "Inquiries",
      inquiryName: "Name",
      inquiryEmail: "Email",
      inquiryPhone: "Phone",
      inquirySubject: "Subject",
      inquiryMessage: "Message",
      inquiryStatus: "Status",
      inquiryDate: "Date",
      statusNew: "New",
      statusInProgress: "In Progress",
      statusCompleted: "Completed",
      
      // Images
      images: "Images",
      uploadImages: "Upload Images",
      selectImage: "Select Image",
      imageGallery: "Image Gallery",
      
      // Analytics
      analytics: "Analytics",
      totalViews: "Total Views",
      uniqueVisitors: "Unique Visitors",
      topPages: "Top Pages",
      topCountries: "Top Countries",
      
      // Common
      actions: "Actions",
      edit: "Edit",
      delete: "Delete",
      cancel: "Cancel",
      save: "Save",
      back: "Back",
      backToDashboard: "Back to Dashboard",
      search: "Search",
      filter: "Filter",
      all: "All",
      yes: "Yes",
      no: "No",
      loading: "Loading...",
      success: "Success",
      error: "Error",
      confirmation: "Confirmation",
      deleteConfirmation: "Are you sure you want to delete this item?",
      
      // Messages
      productSaved: "Product saved successfully",
      productDeleted: "Product deleted successfully",
      categorySaved: "Category saved successfully",
      categoryDeleted: "Category deleted successfully",
      subcategorySaved: "Subcategory saved successfully",
      subcategoryDeleted: "Subcategory deleted successfully",
      inquiryUpdated: "Inquiry updated successfully",
      imageSaved: "Image saved successfully",
      imageDeleted: "Image deleted successfully",
    },
    
    // Legal page
    legal: {
      title: "Legal Information",
      subtitle: "Imprint and Privacy Policy",
      impressum: "Imprint",
      privacy: "Privacy",
      impressumDescription: "Legal information about our company",
      privacyDescription: "How we process your personal data",
      company: "Company",
      address: "Address",
      contact: "Contact",
      businessLicense: "Business License",
      responsiblePerson: "Responsible Person",
      responsiblePersonName: "Manager",
      position: "Position",
      website: "Website",
      websiteDescription: "Official website of Excalibur Power Solutions",
      disclaimer: "Disclaimer",
      disclaimerText: "All information without guarantee. Subject to changes and errors.",
      dataCollection: "Data Collection",
      dataCollectionText: "We collect the following data:",
      dataCollectionItem1: "IP address and browser information",
      dataCollectionItem2: "Contact form data",
      dataCollectionItem3: "Website usage statistics",
      dataCollectionItem4: "Cookies for better user experience",
      dataUsage: "Data Usage",
      dataUsageText: "We use your data for:",
      dataUsageItem1: "Processing your inquiries",
      dataUsageItem2: "Improving our services",
      dataUsageItem3: "Website analysis and optimization",
      dataUsageItem4: "Communication with you",
      cookies: "Cookies",
      cookiesText: "We use cookies to improve your experience and collect anonymous statistics.",
      analytics: "Analytics",
      analyticsText: "We collect anonymous usage data to improve our website.",
      dataProtection: "Data Protection",
      dataProtectionText: "Your data is stored securely and not shared with third parties.",
      userRights: "Your Rights",
      userRightsText: "You have the right to:",
      userRightsItem1: "Information about your stored data",
      userRightsItem2: "Correction of incorrect data",
      userRightsItem3: "Deletion of your data",
      userRightsItem4: "Object to data processing",
      dataRetention: "Data Storage",
      dataRetentionText: "We store your data only as long as necessary.",
      thirdParty: "Third Parties",
      thirdPartyText: "We do not share your data with third parties.",
      changes: "Changes",
      changesText: "We reserve the right to change this privacy policy.",
      contactForPrivacy: "Contact for Privacy",
      contactForPrivacyText: "For privacy questions, contact us:",
      lastUpdated: "Last Updated",
    }
  }
};